...
contract ClaimSourceTest {
    ...
    function testCreateClaimPersistsStableSourceNonce() public {
        uint256 nonce = source.createClaim(
            address(0xB0B), keccak256("USD"), 1_000_000, uint64(block.timestamp + 30 days), keccak256("evidence")
        );
    ...
    function testClaimantCanCancelButCannotCancelTwice() public {
        uint256 nonce =
            source.createClaim(address(0xB0B), keccak256("USD"), 1, uint64(block.timestamp + 1 days), bytes32(0));
