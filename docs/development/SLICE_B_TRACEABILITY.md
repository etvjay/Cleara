...
| Checkpoint | `Checkpoint`, `advanceFinality` | target identity, earlier-block replay, latest-tip preservation, narrow fail-closed range, missing-history, reorg/finality, negative-input, monotonicity, sparse cursor, and populated API route | `VERIFIED_LOCAL` | in-memory for current demo |
| Reorg recovery and replay | `replayReorg`, indexed `BlockHeader` history | trusted-target hash, trusted-parent, missing-history, wrong old hash, wrong target, metadata, multiple affected blocks, later candidate retention, finality, idempotency, malformed replacement, canonical-header preservation tests | `VERIFIED_LOCAL` | local replacement fixture only |
