...
4. `reorg_and_replay`: invalid parent remains blocked, then a replacement with the trusted predecessor is finalized, replayed, promoted, and safely replayed again as a no-op. Earlier-block replay advances through the ordered affected range, retains `REPLAY_REQUIRED` with recovery ownership when the later replacement is missing, and supports explicit trusted-header backfill.
