...
import { createSliceBState, ingestObservation, observationId, projectRelationship, sourceEventId, type ObservationEnvelope } from "../../workers/multichain-execution/src/slice-b.js";
...
const server = createServer((request: IncomingMessage, response: ServerResponse) => {
  ...
  else if (/^\/(facilities|commitments|obligations|settlements|evidence)\/[^/]+$/.test(url.pathname)) {
    const objectId = url.pathname.slice(url.pathname.indexOf("/") + 1).split("/")[1]!;
    value = api.object(objectId);
    if (value === null) { response.writeHead(404); response.end(json({ error: "NOT_INDEXED", objectId })); return; }
  }
