...
try {
  ...
  const wrongResourceType = await get("/facilities/settlement:slice-b-1?relationshipId=relationship:slice-b:fixture"); expect(wrongResourceType.status === 404 && wrongResourceType.body.error === "NOT_INDEXED", "typed route returned the wrong resource type");
