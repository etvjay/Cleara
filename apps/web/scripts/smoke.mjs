...
try {
  ...
  for (const path of ["/try", "/try/replay", "/try/live", "/try/solo", "/try/claim", "/try/multi-party/new"]) {
    const tryPage = await requestRaw(path);
    if (tryPage.status !== 200 || !tryPage.body.includes("src/browser.js")) throw new Error(`try shell is incomplete: ${path}`);
  }
