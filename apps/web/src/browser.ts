...
function detailPanel(stage: CaseStage): string {
  return `<div class="eyebrow">SELECTED STATE</div><h2>${esc(stage.label)}</h2><div class="detail-state">${badge(stage.state)}<span>${esc(stage.domain)}</span></div><p class="detail-copy">${esc(stage.detail)}</p><div class="evidence-title">PROVENANCE / EVIDENCE</div><div class="evidence-list">${stage.evidence.map((item) => `<article class="evidence"><div class="evidence-row"><strong>${esc(item.label)}</strong>${badge(item.kind)}</div><p>${esc(item.detail)}</p><small>Boundary: ${esc(evidenceBoundary(item.kind))}</small><small>${esc(item.source)}${item.artifact ? ` · artifact ${esc(item.artifact)}` : ""}${item.tx ? ` · tx ${esc(item.tx)}` : ""}${item.evidenceId ? ` · evidence ${esc(item.evidenceId)}` : ""}</small></article>`).join("")}</div><div class="boundary-note"><strong>Authority boundary</strong><span>This workbench is a read model. It cannot authorize or submit a financial transition.</span></div>`;
}
