...
import { tryModeIdForPath } from "./try-model.js";

const app = document.querySelector<HTMLElement>("#app");
if (!app) throw new Error("missing #app");

const normalizedPath = window.location.pathname.replace(/\/+$/, "") || "/";
const isTryPath = normalizedPath === "/try" || normalizedPath.startsWith("/try/");
const tryModeId = tryModeIdForPath(normalizedPath);
if (isTryPath) {
  const { mountTryApp } = await import("./try-app.js");
  mountTryApp(app, tryModeId ?? null);
} else {
