...
import { TRY_MODES, tryPathForMode, type TryMode } from "./try-model.js";
...
function modeFor(id: string | null | undefined): TryMode | undefined {
  return id ? TRY_MODES.find((mode) => mode.id === id) : undefined;
}
...
function EnvironmentModes({ activeId }: { activeId: string | null }): ReactElement {
      ...
      <div className="try-mode-buttons">
        {TRY_MODES.filter((mode) => mode.group === group).map((mode) => (
          <a
            ...
            href={tryPathForMode(mode.id)}
            data-try-mode={mode.id}
            aria-current={mode.id === activeId ? "page" : undefined}
            key={mode.id}
          >
...
function LobbyContext(): ReactElement {
  return (
    <aside className="try-mode-context" aria-live="polite">
      <div className="try-context-head">
        <p>WHAT THIS LOBBY GIVES YOU</p>
        <span className="try-context-status">CHOOSE AN INTERFACE</span>
      </div>
      <h2>Enter on your terms.</h2>
      <div className="try-detail-grid">
        <div><small>Understand</small><strong>Replay or watch the configured testnets</strong></div>
        <div><small>Participate</small><strong>Connect a wallet for one bounded action</strong></div>
        <div><small>Route</small><strong>Each mode opens its own product path</strong></div>
        <div><small>Boundary</small><strong>Observation never becomes authority</strong></div>
        <div><small>Next</small><strong>Choose a mode to continue</strong></div>
      </div>
      <div className="try-context-note">No wallet is requested in the lobby. Wallet connection appears only inside a participant route.</div>
    </aside>
  );
}
...
function TryApp({ initialModeId }: { initialModeId: string | null }): ReactElement {
  const mode = modeFor(initialModeId);
  ...
  return (
    <main className="try-page try-page-demo">
      <header className="try-header">
        ...
        <div className="try-header-context"><span className="try-testnet-dot" aria-hidden="true" /><span>TESTNET DEMO</span><a href={mode ? "/try" : "/"}>{mode ? "Back to lobby" : "Back to landing"} <span aria-hidden="true">↗</span></a></div>
      </header>
      <section className="try-intro" aria-labelledby="try-title">
        <p className="try-eyebrow">ENTER CLEARA / {mode ? mode.group : "PARTICIPANT TESTING"}</p>
        <h1 id="try-title">{mode ? mode.title : "Choose how you'll enter the relationship."}</h1>
        <p>{mode ? mode.summary : "Replay the lifecycle, watch the configured testnet heads, or connect a wallet and perform one real participant action."}</p>
      </section>
      ...
      <section className="try-lobby" aria-label="Cleara demo interfaces"><EnvironmentModes activeId={mode?.id ?? null} />{mode ? <ModeContext mode={mode} /> : <LobbyContext />}</section>
      {mode && <section className="try-demo-interface" aria-labelledby="try-interface-title">
...
export function mountTryApp(node: HTMLElement, initialModeId: string | null = null): void {
  createRoot(node).render(<WagmiProvider config={walletConfig}><QueryClientProvider client={queryClient}><RainbowKitProvider><TryApp initialModeId={initialModeId} /></RainbowKitProvider></QueryClientProvider></WagmiProvider>);
}
